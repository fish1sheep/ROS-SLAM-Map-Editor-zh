 ...
 * TypeScript 版本（供 editor.html 使用）。
 *
 * ⚠️ 注意：根目录的 i18n.js 供 index.html（落地页）使用，
...
const LANG_KEY = 'ros_map_editor_lang';
...
export function t(key: string, params?: Record<string, string | number>): string {
  const map = dict[currentLang] || dict['zh'];
  let str: string | undefined = map[key];
  if (str === undefined) {
    // 降级：尝试英文
...
export function setLang(lang: Lang, noSave?: boolean): void {
  ...
  if (typeof document !== 'undefined') {
    ...
    const evt = new CustomEvent('langchange', { bubbles: true, cancelable: false });
...
export function applyTranslations(): void {
  ...
  for (let j = 0; j < titles.length; j++) {
    const el = titles[j] as HTMLElement;
  ...
  for (let k = 0; k < placeholders.length; k++) {
    const el = placeholders[k] as HTMLInputElement;
  ...
  for (let m = 0; m < arias.length; m++) {
    const el = arias[m] as HTMLElement;
  ...
  for (let p = 0; p < alts.length; p++) {
    const el = alts[p] as HTMLImageElement;
  ...
  for (let n = 0; n < btns.length; n++) {
    const el = btns[n] as HTMLElement;
    el.textContent = t('langSwitch');
    el.title = t('langLabel');
  }

  // 更新 <title> 和 meta description（如果标签上有 data-i18n 属性）
  const titleEl = document.querySelector('title[data-i18n]') as HTMLTitleElement | null;
  if (titleEl) {
    const key = titleEl.getAttribute('data-i18n');
    if (key) titleEl.textContent = t(key);
  }

  const descEl = document.querySelector('meta[name="description"][data-i18n]') as HTMLMetaElement | null;
  if (descEl) {
    const key = descEl.getAttribute('data-i18n');
    if (key) descEl.setAttribute('content', t(key));
  }
}
...
// 暴露到全局（向后兼容 index.html 中的 onclick="__i18n.toggleLang()"）
const i18n = { t, getLang, setLang, toggleLang, applyTranslations, init };
